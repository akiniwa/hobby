...
echo 'delete index'
curl -XDELETE 'http://dc:9200/library'
echo '-------------------------------------------';

echo 'create index'
curl -XPUT    'http://dc:9200/library' -d '{
    "settings" : {
        "index" : {
            "number_of_shards" : 1, 
...
echo 'create mapping'
curl -XPUT 'http://dc:9200/library/docs/_mapping?pretty=1' -d '{
    "properties" : {
        "file" : {
            "type" : "attachment",
            "fields" : {
...
echo 'refresh index'
curl -XPOST 'http://dc:9200/library/_refresh?pretty=1'
echo '-------------------------------------------';


echo 'indexed test.doc';
curl -XPOST "http://dc:9200/library/docs?pretty=1" -d '
{
    "file" : {
        "content_type": "doc",
